...
#ifdef  WAC_ENABLE
void
ar6000_wacinfo_event(struct ar6_softc *ar,  A_UINT8 *ptr, A_UINT32 len)
{
    WMI_GET_WAC_INFO *pWacInfo = (WMI_GET_WAC_INFO *)ptr;

    A_MEMCPY(&ar->wacInfo, pWacInfo, sizeof(WMI_GET_WAC_INFO));
...
void ar6000_pspoll_event(AR_SOFTC_T *ar,A_UINT8 aid)
{
    ...
    if (!conn) {
        AR_DEBUG_PRINTF(ATH_DEBUG_ERR,("Can't find this AID\n"));
        return;
    }
