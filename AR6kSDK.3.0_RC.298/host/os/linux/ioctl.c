...
ar6000_xioctl_set_passphrase_cmd(struct net_device * dev, char * userdata)
{
    ...
    if (wmi_set_passphrase_cmd(ar->arWmi, &cmd) == A_OK)
    {
        /* enable WPA offload */
...
ar6000_xioctl_set_excess_tx_retry_thres_cmd(struct net_device * dev, char * userdata)
{
    ...
    if (wmi_set_excess_tx_retry_thres_cmd(ar->arWmi, &cmd) != A_OK)
    {
        ret = -EINVAL;
    }
    return(ret);
...
ar6000_xioctl_wac_ctrl_req_get_cmd(struct net_device * dev, char * userdata, struct ifreq *rq)
{
    ...
    if (wmi_wac_ctrl_req_cmd(ar->arWmi, &cmd) != A_OK)
    {
    	up(&ar->arSem);
    	return -EIO;
    }
    ...
    if (!ret && copy_to_user(rq->ifr_data, &ar->wacInfo, sizeof(WMI_GET_WAC_INFO))) {
            ret = -EFAULT;
    }
	...
	{   
        ...
        case AR6000_XIOCTL_WMI_SET_WPA_OFFLOAD_STATE:
        {
            ...
            if (copy_from_user(&wpaOffloadState, userdata, sizeof(A_UINT8))) {
                ...
                ar->wpaOffloadEnabled = (wpaOffloadState) ? TRUE : FALSE;
            }
#else
            ret = -EOPNOTSUPP;
        ...
        case AR6000_XIOCTL_WMI_ENABLE_WAC_PARAM:
        {
            ...
            } else if (copy_from_user(&cmd, userdata, sizeof(cmd)))
            {
                ...
                if ( cmd.enable & 0x80 ) {
                    cmd.enable &= ~0x80;
                    ar6000_send_generic_event_to_app(ar, WMI_ENABLE_WAC_CMDID, 
                                                    (A_UINT8*)&cmd, sizeof(WMI_WAC_ENABLE_CMD));
                }
                else {
                    if (wmi_wac_enable_cmd(ar->arWmi, &cmd)
                           != A_OK)
                    {
                        ret = -EIO;
                    }
        ...
        case AR6000_XIOCTL_WAC_SCAN_REPLY:
        {
            ...
            } else if (copy_from_user(&cmd, userdata, sizeof(cmd)))
            {
                ...
                if (wmi_wac_scan_reply_cmd(ar->arWmi, cmd.cmdid)
                       != A_OK)
                {
                    ret = -EIO;
                }
        ...
        case AR6000_XIOCTL_WMI_WAC_CTRL_REQ:
        {
            ...
            } else if (copy_from_user(&cmd, userdata, sizeof(cmd)))
            {
                ...
                if ( WAC_SET == cmd.req ) {
                    if (wmi_wac_ctrl_req_cmd(ar->arWmi, &cmd)
                           != A_OK)
                    {
                        ret = -EIO;
                    }
        ...
        case AR6000_XIOCTL_WMI_SET_DIVERSITY_PARAM:
        {
            ...
            {
                   if (wmi_set_div_param_cmd(ar->arWmi, cmd.divIdleTime, cmd.antRssiThresh, cmd.divEnable, cmd.active_treshold_rate)
                           != A_OK)
                    {
                        ret = -EIO;
                    }
