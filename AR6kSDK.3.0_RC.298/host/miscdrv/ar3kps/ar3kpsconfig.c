...
#define WAKEUP_TIMEOUT_DEFAULT_VAL 10
    ...
    for(i = 1; i <numCmds; i++) {
        ...
        if((HciCmdList[i].Hcipacket[4] == CONFIG_PLATFORM) &&
            ...
            if(bit7)
                ...
                (HciCmdList[i].Hcipacket[5] == 0)) {
			A_UCHAR *TLPMHciCmd = HciCmdList[i].Hcipacket;
            cFlags |= TLPM_CONFIG_BIT;
            hdev->IdleTimeout = *((A_UINT16 *)&HciCmdList[i].Hcipacket[IDLE_TIMEOUT_OFFSET + 7]);
    ...
    if(bit7 && !(cFlags & TLPM_CONFIG_BIT)) {
	    ...
	    *((A_UINT16 *)&TLPMHciCmd[IDLE_TIMEOUT_OFFSET + 7]) = hdev->IdleTimeout;
