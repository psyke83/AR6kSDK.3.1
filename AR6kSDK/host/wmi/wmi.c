...
#ifdef P2P
static A_STATUS
wmi_p2p_goneg_event_rx(struct wmi_t *wmip, A_UINT8 *datap,int len);
#endif
...
wmi_control_rx(struct wmi_t *wmip, void *osbuf)
{
    ...
    case (WMI_P2P_GO_NEG_RESULT_EVENTID):
         status = wmi_p2p_goneg_event_rx(wmip, datap, len) ;
         break;
#endif
...
wmi_sync_point(struct wmi_t *wmip)
{
    ...
    for (i=0; i < WMM_NUM_AC ; i++) {
        if (pWmiPriv->wmi_fatPipeExists & (1 << i)) {
            numPriStreams++;
    ...
    do {
         ...
         */
        cmd->dataSyncMap = pWmiPriv->wmi_fatPipeExists;
