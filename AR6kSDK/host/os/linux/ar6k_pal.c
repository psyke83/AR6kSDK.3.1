...
static int btpal_send_frame(struct sk_buff *skb)
{
	...
	do {
		if(type == HCI_COMMAND_TYPE)
		{
			...
			if (wmi_send_hci_cmd(ar->arDev[0]->arWmi, skb->data, skb->len) != A_OK)
			{
		...
		else if(type == HCI_ACL_TYPE)
		{
			...
			PRIN_LOG("\nAdd WMI header");
			if (wmi_data_hdr_add(ar->arDev[0]->arWmi, osbuf, DATA_MSGTYPE, 0, WMI_DATA_HDR_DATA_TYPE_ACL,0,NULL) != A_OK) {
				PRIN_LOG("XIOCTL_ACL_DATA - wmi_data_hdr_add failed\n");
			} else {
				/* Send data buffer over HTC */
				PRIN_LOG("acl data tx");
				ar6000_acl_data_tx(osbuf, ar->arDev[0]);
			}
			txSkb = NULL;
		}
	} while (FALSE);
