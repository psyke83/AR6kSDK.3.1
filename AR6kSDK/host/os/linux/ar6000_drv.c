...
ar6000_parse_dev_mode(A_CHAR *mode)
{
    ...
    if(!match) {
        num_device = fwmode = 1;
        A_PRINTF("ERROR: Wrong mode. using default (single device STA mode).\n");
        return;
    }
...
ar6000_transfer_bin_file(AR_SOFTC_T *ar, AR6K_BIN_FILE file, A_UINT32 address, A_BOOL compressed)
{
	...
	if (file==AR6K_BOARD_DATA_FILE && fw_entry->data) {
	#ifdef SOFTMAC_FILE_USED
	        ar6000_softmac_update(ar, (A_UCHAR *)fw_entry->data, fw_entry_size);
	#endif
	        if (regcode!=0) {
	            ar6000_reg_update(ar, (A_UCHAR *)fw_entry->data, fw_entry_size, regcode);
	        }
	}
...
ar6000_restart_endpoint(AR_SOFTC_T *ar)
{
    ...
    for(i = 0; i < num_device; i++) {
        arPriv = ar->arDev[i];
        if (arPriv->arSsidLen && ar->arWlanState == WLAN_ENABLED) {
...
ar6000_stop_endpoint(AR_SOFTC_T *ar, A_BOOL keepprofile, A_BOOL getdbglogs)
{
    ...
    for(i = 0; i < num_device; i++)
    {
        ...
        if (ar->arWmiReady == TRUE)
        {
            if (!bypasswmi) {
                ...
                ar->arWmiReady  = FALSE;
                //Brian should don't stop wmi, just mark it to un ready
                //wmi_shutdown(arPriv->arWmi);
                arPriv->arWmiEnabled = FALSE;
               // arPriv->arWmi = NULL;
               /*
    ...
    * a debug session */
        ...
        if (ar->arHifDevice != NULL) {
             ...
             * on that chip (it stops responding to SDIO inputs if it is cold reset and the power
             * to the SDIO bus remains on.
             */
        ...
        switch (pConnect->ServiceID) {
            case WMI_CONTROL_SVC :
                if(!bypasswmi)
                {
                    ...
                    wmi_set_control_ep(NULL, response.Endpoint);
    ...
    do {
        ...
        if(csumOffload && (csum==CHECKSUM_PARTIAL)){
            ...
            if(csumOffload && (csum ==CHECKSUM_PARTIAL)){
                ...
                if (wmi_data_hdr_add(arPriv->arWmi, skb, DATA_MSGTYPE, bMoreData, dot11Hdr,
...
#endif
            {
                if (wmi_data_hdr_add(arPriv->arWmi, skb, DATA_MSGTYPE, bMoreData, dot11Hdr,0,NULL) != A_OK) {
                    AR_DEBUG_PRINTF(ATH_DEBUG_ERR,("ar6000_data_tx - wmi_data_hdr_add failed\n"));
                    break;
                }
            }
...
#endif
                    if ((arPriv->arNetDev->flags & IFF_UP) == IFF_UP) {
                        if (arPriv->arNetworkType == AP_NETWORK) {
...
ar6000_set_multicast_list(struct net_device *dev)
{
    ...
    if ((!enableAll) && (!disableAll))  {
         ...
         */
#if LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 35)
        for (j = 0, mc = dev->mc_list; mc && (j < dev->mc_count); j++, mc = mc->next) {
            mac = mc->dmi_addr;
#else
...
ar6000_get_iwstats(struct net_device * dev)
{
    ...
    if (in_atomic())
    {
        wmi_get_stats_cmd(arPriv->arWmi);
...
ar6000_ready_event(void *devt, A_UINT8 *datap, A_UINT8 phyCap, A_UINT32 sw_ver, A_UINT32 abi_ver)
{
    ...
    for(i = 0; i < num_device ; i++) {
        ...
        A_PRINTF("DEV%d mac address = %2.2x:%2.2x:%2.2x:%2.2x:%2.2x:%2.2x\n",
...
add_new_sta(AR_SOFTC_DEV_T *arPriv, A_UINT8 *mac, A_UINT16 aid, A_UINT8 *wpaie,
            A_UINT8 ielen, A_UINT8 keymgmt, A_UINT8 ucipher, A_UINT8 auth, A_UINT8 wmode)
{
    AR_SOFTC_T  *ar = arPriv->arSoftc;
    AR_SOFTC_AP_T *arAp = &arPriv->arAp;
...
void
ar6000_connect_event(AR_SOFTC_DEV_T *arPriv, A_UINT16 channel, A_UINT8 *bssid,
                     ...
                     A_UINT8 *assocInfo)
{
    ...
    if(arPriv->arNetworkType & AP_NETWORK) {
        ...
        if(A_MEMCMP(dev->dev_addr, bssid, ATH_MAC_LEN)==0) {
            arPriv->arBssChannel = channel;
            ik = &arAp->ap_mode_bkey;

            A_PRINTF("AP%d: [UP] SSID %s MAC %2.2x:%2.2x:%2.2x:%2.2x:%2.2x:%2.2x\n",
...
*/
        ...
        aid         = channel & 0xFF;
        wmode       = (channel >> 8) & 0xFF;
        keymgmt     = listenInterval & 0xFF;
        auth_alg    = (listenInterval >> 8) & 0xFF;

        A_PRINTF("NEW STA %2.2x:%2.2x:%2.2x:%2.2x:%2.2x:%2.2x \n "
            " AID=%d AUTH=%d WMODE=%d KEYMGMT=%d CIPHER=%d\n",
            bssid[0], bssid[1], bssid[2], bssid[3], bssid[4], bssid[5],
            aid, auth_alg, wmode, keymgmt, beaconInterval);

        add_new_sta(arPriv, bssid, aid, wpaie, wpaLen, keymgmt,
            beaconInterval /* cipher */, auth_alg, wmode);
    ...
    arSta = &arPriv->arSta;
#ifdef ATH6K_CONFIG_CFG80211
    ar6k_cfg80211_connect_event(arPriv, channel, bssid,
                        ...
                        A_UINT8 assocRespLen, A_UINT8 *assocInfo, A_UINT16 protocolReasonStatus)
{
    ...
    A_PRINTF("Disconnect Reason is %d, Status Code is %d", reason, protocolReasonStatus);
...
ar6000_aggr_rcv_addba_resp_evt(AR_SOFTC_DEV_T *arPriv, WMI_ADDBA_RESP_EVENT *evt)
{
    ...
    A_PRINTF("ADDBA RSP: tid=%d, connid=%d, status=%d, sz=%d\n", tid, connid, evt->status, evt->amsdu_sz);
    if(conn) {
...
void ar6000_pspoll_event(AR_SOFTC_DEV_T *arPriv,A_UINT8 aid)
{
    ...
    if(!conn) {
        A_PRINTF("PS-POLL from invalid STA\n");
        return;
    }
        ...
        if(temp_priv->arConnected) {
            ...
            if(arPriv->arChannelHint) {
                if(temp_priv->arBssChannel != arPriv->arChannelHint) {
                    A_PRINTF("Error: Channel should be %d MHz\n", temp_priv->arBssChannel);
                    return 0;
                }
            } else {
