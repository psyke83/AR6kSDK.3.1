                     ...
                     struct iw_point *data, char *ssid)
{
    ...
    A_UINT8     i=0;
    A_UINT8 prevMode = arPriv->arNetworkType;
            ...
            __u32 *mode, char *extra)
{
    ...
    if(ar->arConfNumDev != 1) {
        A_PRINTF("Mode switch not allowed in concurrent mode\n");
        return 0;
    }
    /*
    ...
    if (arPriv->arNetworkType != arPriv->arNextMode) {
        if ((arPriv->arNetworkType == AP_NETWORK) && arPriv->arConnected) {
            /* We are switching from AP to STA | IBSS mode, Switch off the AP */
            wmi_disconnect_cmd(arPriv->arWmi);
        }
    ...
    if (data->pointer && (data->length == sizeof(struct iw_scan_req)))
    {
        if ((data->flags & IW_SCAN_THIS_ESSID) == IW_SCAN_THIS_ESSID)
        {
            ...
            if (copy_from_user(&req, data->pointer, sizeof(struct iw_scan_req)))
                return -EIO;
            if (wmi_probedSsid_cmd(arPriv->arWmi, 1, SPECIFIC_SSID_FLAG, req.essid_len, req.essid) != A_OK)
                return -EIO;
        ...
        else
        {
            if (arSta->scanSpecificSsid) {
                if (wmi_probedSsid_cmd(arPriv->arWmi, 1, DISABLE_SSID_FLAG, 0, NULL) != A_OK)
                    return -EIO;
