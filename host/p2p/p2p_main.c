...
static A_STATUS p2p_parse_wps_ie(const A_UINT8 *data, A_UINT8 len, struct p2p_ie *p2p_ie)
{
    ...
    if(attr.primary_dev_type) {
        p2p_ie->pri_dev_type = attr.primary_dev_type;
 ...
 */
static A_STATUS p2p_parse_ies(const A_UINT8 *data, A_UINT32 len, struct p2p_ie *p2p_ie, A_UINT8 ietype)
{
    const A_UINT8 *frm, *efrm;
    A_STATUS status = A_ENODEV;
    A_BOOL found = FALSE;
    ...
    while (frm < efrm) {
        switch (*frm) {
        case IEEE80211_ELEMID_VENDOR:
            if(iswscoui(frm) && (ietype == PARSE_WPS_IE)) {
                status = p2p_parse_wps_ie(frm+6, frm[1]-4, p2p_ie);
                found = TRUE;
            } else if(isp2poui(frm) && (ietype == PARSE_P2P_IE)) {
                found = TRUE;
                status = p2p_parse_p2p_ie(frm+6, frm[1]-4, p2p_ie);
            }
            break;
...
static A_STATUS p2p_parse(WMI_BI_FTYPE fType, const A_UINT8 *data, A_UINT32 len, struct p2p_ie *p2p_ie)
{
    ...
    status = p2p_parse_ies(data, len, p2p_ie, PARSE_P2P_IE);
...
void *p2p_bssinfo_rx(void *ctx, WMI_BI_FTYPE fType, A_UINT8 *addr, A_UINT16 channel, const A_UINT8 *data, A_UINT32 len)
{
    ...
    if (status != A_OK) {
        ...
        if (fType == BEACON_FTYPE || fType == PROBERESP_FTYPE) {
            ...
            struct p2p_ie wps_msg;
            A_MEMZERO(&wps_msg, sizeof(struct p2p_ie));
            ...
            status = p2p_parse_ies(data, len, &wps_msg, PARSE_WPS_IE);
            if (status != A_OK) {
                /* No WPS IE or Bad WPS IE */
                return NULL;
            } else {

                A_MEMCPY(peer_dev->dev.device_name, wps_msg.device_name, sizeof(peer_dev->dev.device_name));

                if (wps_msg.pri_dev_type) {
                    A_MEMCPY(peer_dev->dev.pri_dev_type, wps_msg.pri_dev_type,
            ...
            A_WMI_P2PDEV_EVENT(p2p_dev_ctx->dev, addr, peer_dev->dev.p2p_device_addr,
                ...
                wps_msg.device_name, wps_msg.dev_name_len,
                wps_msg.config_methods,
                (msg.capability) ? msg.capability[0]:0, (msg.capability)?msg.capability[1]:0);
...
A_STATUS p2p_prov_disc_req(void *ctx, WMI_P2P_PROV_DISC_REQ_CMD *prov_disc_req)
{
    ...
    if (peer == NULL) {
        /* Unknown peer device.
         */
        peer = p2p_get_device_intf_addrs(ctx, prov_disc_req->peer);
    }
