...
ar6000_ready_event(void *devt, A_UINT8 *datap, A_UINT8 phyCap, A_UINT32 sw_ver, A_UINT32 abi_ver)
{
    ...
    for(i = 0; i < num_device ; i++) {
...
#if 1	/*E_BOOK*/
		wmi_scanparams_cmd(arPriv->arWmi,arSta->scParams.fg_start_period,
		...
		RoamCtrl.info.lrScanParams.lowrssi_scan_period = 0;
		RoamCtrl.info.lrScanParams.lowrssi_scan_threshold = 0;
		RoamCtrl.info.lrScanParams.lowrssi_roam_threshold = 0;
		RoamCtrl.info.lrScanParams.roam_rssi_floor = 0;
		wmi_set_roam_ctrl_cmd(arPriv->arWmi, &RoamCtrl,sizeof(RoamCtrl));	//ROAM OFF
#endif
    }
}
...
#else	/* set scan parameters*/
		arSta->scParams.fg_start_period = 0xFFFF;	//disable
		arSta->scParams.fg_end_period = 0;
		arSta->scParams.bg_period = 0xFFFF;			//disable
		arSta->scParams.minact_chdwell_time = 100;
		arSta->scParams.maxact_chdwell_time = 105;
		arSta->scParams.pas_chdwell_time = 0;
		arSta->scParams.shortScanRatio = 0;
		arSta->scParams.scanCtrlFlags = CONNECT_SCAN_CTRL_FLAGS | SCAN_CONNECTED_CTRL_FLAGS | ACTIVE_SCAN_CTRL_FLAGS;
		arSta->scParams.max_dfsch_act_time = 0;
		arSta->scParams.maxact_scan_per_ssid = 1;
#endif
