...
#define WOW_ENABLE_MAX_INTERVAL 0	/*Don't define.(Because a listen_interval & BISS will be overwritten)*/
#define WOW_SET_SCAN_PARAMS     0	/*Don't define.(Because a scan params will be overwritten)*/

int HTC_credit_report_has_arrived = 0;
DECLARE_WAIT_QUEUE_HEAD(HTC_credit_report_queue);
    ...
    do {
        ...
        if (state == WLAN_ENABLED) {
            ...
            for(i = 0; i < num_device; i++)
            {
                ...
                HTC_credit_report_has_arrived = 0; /* Mark credit report as having NOT arrived yet */
                if ((status=wmi_set_host_sleep_mode_cmd(arPriv->arWmi, &hostSleepMode))!=A_OK) {
                    break;
                }
                
                /*
                 * Wait for the HTC credit report to come in before suspending.  Failing to do so could
                 * cause a kernel crash (if we get an interrupt while the kernel is going into suspend).
                 */

                do {
                    wait_event_interruptible_timeout(HTC_credit_report_queue, (HTC_credit_report_has_arrived == 1), HZ);
                } while (!HTC_credit_report_has_arrived);
