...
static struct wake_lock ar6k_aggr_wake_lock;
#endif
...
{
#ifdef CONFIG_HAS_WAKELOCK
    wake_lock_init( &ar6k_aggr_wake_lock, WAKE_LOCK_SUSPEND, "ar6000 aggr");
#endif
...
aggr_module_destroy(void)
{
    if(p_aggr) {
        ...
        p_aggr = NULL;
#ifdef CONFIG_HAS_WAKELOCK
        wake_lock_destroy( &ar6k_aggr_wake_lock );
#endif
    }
}
...
#ifdef CONFIG_HAS_WAKELOCK
                wake_lock_timeout( &ar6k_aggr_wake_lock, msecs_to_jiffies(AGGR_RX_TIMEOUT+200) );
#endif
                rxtid->progress = FALSE;
                rxtid->timerMon = TRUE;
                break;
